...
1. In the Apple Developer account, register the app's bundle ID, enable iCloud/CloudKit and Push Notifications, and associate the intended iCloud container. Use the same container and CloudKit environment on both Macs.
2. Set the signing team and identity in `Config/Local.xcconfig`, then set `GLOSSA_CODE_SIGN_ENTITLEMENTS = Glossa/Glossa-iCloud.entitlements` and `GLOSSA_ICLOUD_CONTAINER = iCloud.YOUR_REGISTERED_CONTAINER`. The matching provisioning profile must include both CloudKit and push entitlements. Keep personal values out of tracked files.
3. Build with the README Release command. Add `-allowProvisioningUpdates` if Xcode should retrieve or update the profile using its signed-in developer account. If the team has no registered Mac, first build with destination `platform=macOS` and `-allowProvisioningDeviceRegistration` to register the current development Mac. Development builds use `development` / `Development`; distribution profiles use `production` / `Production` for `GLOSSA_APS_ENVIRONMENT` / `GLOSSA_ICLOUD_ENVIRONMENT`. Deploy the development CloudKit schema to Production in CloudKit Console before distributing a production build.
4. Open **Wordbook → Sync & Backups**, enable sync, then quit and reopen Glossa. It uses the existing local store. Disabling sync also takes effect after restart and keeps the local store; it does not delete cloud records.

Sync is opt-in and uses the user's private CloudKit database. CloudKit/Core Data events refresh the wordbook without polling. The sync sheet checks `CKContainer.accountStatus`, rechecks after `CKAccountChanged`, and explains missing accounts, restrictions, offline connections, full storage, and provisioning failures without displaying raw errors or record contents. Successful import/export events show separate session activity times; setup completion is not reported as an upload. A successful operation does not hide a failure in another operation type. Account-change notifications clear the previous account's displayed activity times and reload records; account/data reconciliation remains managed by SwiftData/CloudKit.

**Check iCloud Status** checks account availability and refreshes the local snapshot; it does not force an upload. Transfer scheduling and retries belong to CloudKit. An observed completed event is not proof that every device has received every record. Real two-device sync, offline edits, account transitions, deletion and restore, and backup recovery remain unvalidated. See Apple's [SwiftData sync setup](https://developer.apple.com/documentation/swiftdata/syncing-model-data-across-a-persons-devices) and [CloudKit account status](https://developer.apple.com/documentation/cloudkit/ckcontainer/accountstatus(completionhandler:)) documentation.
